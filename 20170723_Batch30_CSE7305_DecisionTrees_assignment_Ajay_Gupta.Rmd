...
```{r}
setwd("E:/Course_Data/July23")
ilpd_Data = read.csv("ilpd_data.csv", sep = ",", header = T)
...
test_data = knnImputation(data = test_data, k = 5)
...
buildingData = read.csv("building_energy.csv",header = T, sep = ",")
...
train_rows <- sample(1:nrow(buildingData), 0.7*nrow(buildingData))

train_data <- buildingData[train_rows, ]

test_data <- buildingData[-train_rows, ]
...
regressionTree <- rpart(heating_load ~ ., train_data)
...
preds <- predict(regressionTree, test_data)
...
regr.eval(test_data$heating_load, preds)
